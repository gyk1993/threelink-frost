function right_stance_constraints(nlp, bounds, varargin)
    ...
    domain.VirtualConstraints.time_vc.imposeNLPConstraint(nlp, [bounds.time.kp, bounds.time.kd], [1,1]);
    
    % tau boundary [0,1]
    T_name = lower(nlp.OptVarTable.T(1).Name);
    T = SymVariable(T_name,[nlp.OptVarTable.T(1).Dimension,1]);
    p_name = lower(nlp.OptVarTable.ptime(1).Name);
    p = SymVariable(p_name,pnlp.OptVarTable.ptime(1).Dimension,1])
    ...
    % output boundary
    y_pos = domain.VirtualConstraints.time_vc;
